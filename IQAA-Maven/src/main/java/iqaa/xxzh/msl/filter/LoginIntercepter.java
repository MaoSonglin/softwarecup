		...
		if(user == null){
			...
			for(Cookie c : cookies){
				...
				if("autoLogin".equals(name)){
					...
					// 查询用户信息
					DetachedCriteria criteria = DetachedCriteria.forClass(UserBean.class).add(Restrictions.eq("username", split[0]));
					...
					if(!list.isEmpty()){
						...
						if(user2.getPassword().equals(split[1])&&!user2.getIsLock()){
							...
							Map<UserBean,HttpSession> map = (Map<UserBean, HttpSession>) app.getAttribute(UserBean.USER_MAP);
